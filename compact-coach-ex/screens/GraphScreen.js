    ...
    try {
      if (weight && !isNaN(weight)) {
        ...
        if (newWeightEntries.length > 0) {
          let totalWeightLoss =
            newWeightEntries[newWeightEntries.length - 1].weight -
            newWeightEntries[0].weight;
          await AsyncStorage.setItem(
            "totalWeightLoss",
            JSON.stringify(totalWeightLoss)
          );
      ...
      <ScrollView>
        {weightEntries.map((entry, index) => (
          <TouchableOpacity
            ...
            accessibilityLabel="Tap to Remove Entries"
          >
            <Text
              style={styles.entryText}
            >{`Date: ${entry.date}, Weight: ${entry.weight}`}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
